...
namespace InternshipProject_2.Manager
{
    public class TicketManager : ITicketManager
    {
        ...
        {
            var map = MapperConfig.InitializeAutomapper();
            var dbTicket = await _context.Tickets.Include(i => i.Reporter).Include(i => i.Comments).Include(i => i.Watchers.Where(w => w.IsDeleted == false)).Include(i => i.Watchers).Include(i=>i.Attachements).ToListAsync();
